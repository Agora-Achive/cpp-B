...
### 출력 
 ...
 <p>
   우선, 전체적인 도화지(가로100, 세로100)를 하나의 모눈종이로 생각한다. 
   한 칸을 넓이 1로 생각하여 좌표를 돌며 그 좌표에 색칠되어 있다면,<br>
   2차원 배열(도화지 모눈종이) 중, 그 좌표에 해당되는 값에 1을 넣어준다.<br>
   (1을 넣어준다는 것은 그 자리에 색칠되어 있다는 것을 의미한다.)<br>
   겹치는 넓이를 생각하지 않아도 돼서 복잡하지 않다.<br>
   그렇게 색종이의 수만큼 돌고, 2차원 배열 안에 1이 몇 개 들어있는지 count하면 색종이가 색칠된 넓이가 나온다.
 </p>
